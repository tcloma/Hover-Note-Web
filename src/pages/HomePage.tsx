...
const HomePage = ({
   ...
   const splitDirName = hasInitDir
      ? dirName.split('/')
      : dirName.toString().split('\\');
   ...
   const uniqueObjArray = [
      // ...new Map(dirFiles.map((item) => [item['name'], item])).values(),
   ];
   ...
   return (
      <>
         ...
         >
            ...
            {/* {dirFiles.length === 0 ? (
               <Text fontSize='5xl' color='whiteAlpha.900'>
                  {' '}
                  This directory has no supported files{' '}
               </Text>
            ) : (
